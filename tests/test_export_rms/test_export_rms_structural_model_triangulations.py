"""Test the dataio running RMS utility function for fault surface triangulations."""
...
TRIANG_NODES = \
    Path("tests/data/drogon/rms/output/structural_model/points/f1--extractedfaultpoints_tri_nodes.csv").absolute()
TRIANG_TOPOLOGY = \
    Path("tests/data/drogon/rms/output/structural_model/points/f1--extractedfaultpoints_tri_triang.csv").absolute()


@pytest.fixture(scope="package")
def triang_nodes():
    return pd.read_csv(TRIANG_NODES)

@pytest.fixture(scope="package")
def triang_topology():
    return pd.read_csv(TRIANG_TOPOLOGY)


@pytest.fixture
def export_structural_model_triangulations(
...
@inside_rms
def test_rms_structural_model_triangulations_export_class(
    export_structural_model_triangulations):
    ...
    assert export_structural_model_triangulations._volume_table_name == "geogrid_volumes"
